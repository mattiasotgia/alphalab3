...
void filtro_passa_alto_attivo(){

std::ifstream file ("../dati/filtro_passa_alto_attivo.txt");
...
TF1* fit_bode = new TF1("fit_bode","[0]/sqrt([1]/pow(x,2)+1)");  //[0]=G [1]=nu_0^2
fit_bode->SetParName(0,"G");
fit_bode->SetParName(1,"v_0^2");
//fit_bode->SetParameters();

TF1* fit_fase = new TF1("fit_fase","-atan([0]/x)"); //[0]=nu_0
...
gStyle->SetOptFit();
c1->SaveAs("../figure/passaalto_attivo_bode_H.pdf");
...
gStyle->SetOptFit();
c2->SaveAs("../figure/passaalto_attivo_bode_fase.pdf");
