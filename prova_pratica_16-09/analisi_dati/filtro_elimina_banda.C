...
void filtro_elimina_banda(){

std::ifstream file ("../dati/filtro_elimina_banda.txt");
...
TF1* fit_bode = new TF1("fit_bode","1/sqrt(1+(pow(x,2)*[0])/([1]*(([0]-pow(x,2)))^2))");  //[0]=w0^2 [1]=Q^2
fit_bode->SetParName(0,"v_0^2");
fit_bode->SetParName(1,"Q^2");
//fit_bode->SetParameters();

TF1* fit_fase = new TF1("fit_fase","-atan(([0]*x)/([1]*([0]^2-pow(x,2))))"); //[0]=w0 [1]=Q
...
gStyle->SetOptFit();
c1->SaveAs("../figure/elimina_banda_bode_H.pdf");
...
gStyle->SetOptFit();
c1->SaveAs("../figure/elimina_banda_bode_fase.pdf");

double nu0_b = sqrt(fit_bode->GetParameter(0));
double e_nu0_b = 2*nu0_b*(fit_bode->GetParError(0));
...
if(abs(nu0_b-nu0_f) < 3*sqrt(pow(e_nu0_b,2)+pow(e_nu0_f,2))){
    std::cout<<"I valori dei parametri sono compatibili e la miglior stima è:"<<best_value_nu0<<" +/- "<<e_b_v_nu0<<std::endl;
}
else{
